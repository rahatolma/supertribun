...
export async function fetchShareData(id){
  ...
  const response=await fetch(`${base.replace(/\/$/,'')}/rest/v1/rpc/get_prediction_share`,{
    method:'POST',headers:{apikey:key,Authorization:`Bearer ${key}`,'Content-Type':'application/json'},
    body:JSON.stringify({p_share_id:id}),signal:AbortSignal.timeout(8000),
  });
  ...
  if(!data||data.id!==id||!UUID.test(data.match_id)||
    ...
    !Number.isFinite(Date.parse(data.kickoff_at))||!Number.isFinite(Date.parse(data.locked_at))||
