...
test('Vercel routes expose one dynamic share page, OG image and Apple association',async()=>{
 ...
 assert.match(vercel,/"\/t\/:id"/);assert.match(page,/og:image/);assert.match(page,/share_open_app_click/);assert.match(page,/\/t\/\[id\]/);assert.doesNotMatch(page,/cookieConsent|Çerez tercihleri|mağaza bağlantıları yakında/i);assert.match(image,/ImageResponse/);assert.doesNotMatch(image,/runtime:'edge'|headers:\{'Cache-Control'/);
